	...
	function navigate(page: number) {
		if (page > 5) {
			let userConfirmation = window.confirm(
				"You are navigating to a page number greater than 5. Developer API key doesn't allow querying those. Do you wish to continue?"
			);
			if (!userConfirmation) {
	...
	{#if data.queriedNews.length > 0}
		<div class="flex flex-col justify-center items-center gap-1 mt-4">
			<div class="flex gap-3 parent">
				<button
					class="btn variant-outline-secondary variant-filled div1"
				...
				<button
					class="btn variant-outline-secondary variant-filled div2"
					on:click={() => navigate(currentPage + 1)}
		...
		<div class="flex flex-col justify-center items-center gap-1 mt-4">
			<div class="flex gap-3">
				<button
					class="btn variant-outline-secondary variant-filled"
					on:click={() => navigate(currentPage - 1)}
					disabled={currentPage === 1}>Previous</button
				>
				<ul class="flex">
